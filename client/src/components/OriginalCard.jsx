...
const GRADE_COLOR = { 'A+': '#34D399', A: '#34D399', 'A-': '#6EE7B7', 'B+': '#86EFAC', B: '#A3E635', 'B-': '#BEF264', 'C+': '#FDE047', C: '#FCD34D', 'C-': '#FBBF24', 'D+': '#FB923C', D: '#F87171', F: '#EF4444' };
...
function ProspectListView({ prospects }) {
  if (!prospects || !prospects.length) return null;
  return (
        ...
        <div key={i} style={{ background: '#0A0F1A', border: '1px solid #1A2535', borderRadius: 6, padding: '10px 14px', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#F0EDE6', marginBottom: 3 }}>
              {p.name}
              {p.stars && <span style={{ fontSize: 11, color: '#C4973A', marginLeft: 6 }}>{'★'.repeat(p.stars)}</span>}
            </div>
  ...
  return (
    <div className="pr-card" style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.12em', background: `rgba(255,255,255,0.06)`, color: typeInfo.color, padding: '3px 9px', borderRadius: 3 }}>
          {typeInfo.label}
        </span>
        {onSave && (
